...
class ImageAsteroid : public Asteroid {
public:
	ImageAsteroid(int w, int h) : Asteroid(w, h) {
		baseDamage = 8;
		texture = LoadTexture("asteroid_image.png");
		scale = 0.15f;
	}

	~ImageAsteroid() override {
...
// Shape selector
enum class AsteroidShape { TRIANGLE = 3, SQUARE = 4, PENTAGON = 5, IMAGE = 6, RANDOM = 0 };
...
static inline std::unique_ptr<Asteroid> MakeAsteroid(int w, int h, AsteroidShape shape) {
	...
	case AsteroidShape::IMAGE:
		return std::make_unique<ImageAsteroid>(w, h);
	default: {
...
enum class WeaponType { LASER, BULLET, GREEN_STAR, COUNT };
class Projectile {
public:
	...
	void Draw() const {
		// Oblicz kąt kierunku lotu
		float angle = atan2f(physics.velocity.y, physics.velocity.x) * RAD2DEG + 90.0f;

		// Parametry wielkości rakiety
		float size = 10.0f;
		if (type == WeaponType::BULLET) {
			DrawCircleV(transform.position, 5.f, WHITE);
		}
		else if (type == WeaponType::GREEN_STAR) {
			// Rysuj pięcioramienną gwiazdkę
			int points = 10; // 5 ramion = 10 punktów (na przemian: wierzchołki zewnętrzne i wewnętrzne)
			float radiusOuter = size;
			float radiusInner = size * 0.4f;

			Vector2 star[10];
			for (int i = 0; i < points; i++) {
				float currentAngle = DEG2RAD * (angle + i * 360.0f / points);
				float radius = (i % 2 == 0) ? radiusOuter : radiusInner;

				star[i] = {
					transform.position.x + sinf(currentAngle) * radius,
					transform.position.y - cosf(currentAngle) * radius
				};
			}

			// Rysuj gwiazdkę jako wielokąt (linia łącząca wszystkie punkty)
			for (int i = 0; i < points; i++) {
				DrawLineV(star[i], star[(i + 1) % points], GREEN);
			}

			// Efekt płomienia za rakietą
			DrawCircleV(
...
inline static Projectile MakeProjectile(WeaponType wt, const Vector2 pos, float speed)
{
	...
	else if (wt == WeaponType::GREEN_STAR) {
		return Projectile(pos, vel, 100, wt); // lub inna wartość obrażeń
	}
	// Domyślnie LASER
	return Projectile(pos, vel, 20, WeaponType::LASER);
}
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			if (IsKeyPressed(KEY_EIGHT)) {
				currentShape = AsteroidShape::IMAGE;
			}
			...
			// Render everything
			{
				...
				(currentWeapon == WeaponType::BULLET) ? "BULLET" : "GREEN_STAR";
